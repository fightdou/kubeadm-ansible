...
def sort_osd_crush_location(location, module):
    ...
    try:
        ...
        return sorted(location, key=lambda crush: crush_bucket_types.index(
                      crush[0]))
    except ValueError as error:
        fatal("{} is not a valid CRUSH bucket, valid bucket types are "
              "{}".format(error.args[0].split()[0],
                          crush_bucket_types), module)
...
def create_and_move_buckets_list(cluster, location, containerized=None):
    ...
    for item in location:
        ...
        cmd_list.append(generate_cmd(cluster, "add-bucket", bucket_name,
                        bucket_type, containerized))
        if previous_bucket:
            # ceph osd crush move monrack root=maroot
            cmd_list.append(generate_cmd(cluster, "move", previous_bucket,
                            "%s=%s" % (bucket_type, bucket_name),
                            containerized))
...
def run_module():
    ...
    rc, cmd, out, err = exec_commands(module, create_and_move_buckets_list(
        cluster, location, containerized))
